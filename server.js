...
async function getSettings() {
  ...
  try {
    ...
    const cfg = {
      ...
      gmail_user:         map.gmail_user          || (process.env.GMAIL_USER         || "").trim(),
      gmail_pass:         map.gmail_pass          || (process.env.GMAIL_PASS         || "").trim(),
    ...
    return {
      ...
      gmail_pass:         (process.env.GMAIL_PASS          ||"").trim(),
...
async function sendEmail(to, subject, htmlBody, bookingId, type) {
  ...
  const user = cfg.gmail_user, pass = cfg.gmail_pass;
  const logEntry = { id: uuidv4(), to_email: to, subject, status: "skipped", type: type||"general", booking_id: bookingId||null };

  if (!user || !pass || user.length < 5) {
    console.log("ℹ️  Email skipped — Gmail not configured");
    await supabase.from("email_logs").insert({ ...logEntry, status: "skipped", error: "Gmail not configured" });
    return { skipped: true };
  }
  ...
  try { nodemailer = require("nodemailer"); }
    ...
    await supabase.from("email_logs").insert({ ...logEntry, status: "failed", error: "nodemailer not installed" });
  ...
  try {
    ...
    await supabase.from("email_logs").insert({ ...logEntry, status: "sent" });
    return { messageId: info.messageId };
  } catch (e) {
    console.error("Email error:", e.message);
    await supabase.from("email_logs").insert({ ...logEntry, status: "failed", error: e.message });
...
async function emailNewBooking(booking) {
  const cfg = await getSettings();
  const adminTarget = cfg.admin_email || cfg.gmail_user;
...
app.get("/api/settings", adminAuth, async (req, res) => {
  try {
    ...
    res.json({
      ...
      // Email
      gmail_user:  { set: cfg.gmail_user.length > 3,  value: cfg.gmail_user },
      gmail_pass:  { set: cfg.gmail_pass.length > 3,  masked: mask(cfg.gmail_pass) },
      ...
      // Summary
      channels: {
        email:     cfg.gmail_user.length > 3 && cfg.gmail_pass.length > 3,
...
app.put("/api/settings/:key", adminAuth, async (req, res) => {
  try {
    ...
    const allowed = ["gemini_api_key","admin_email","gmail_user","gmail_pass","telegram_bot_token","telegram_chat_id","twilio_account_sid","twilio_auth_token","twilio_from","admin_whatsapp"];
...
app.listen(PORT, () => {
  ...
  console.log(`📧 Email: ${process.env.GMAIL_USER?"✅ env var set":"Check Supabase settings table"}`);
});
