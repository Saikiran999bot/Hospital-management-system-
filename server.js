...
async function sendEmail(to, subject, htmlBody, bookingId, type) {
  ...
  if (!apiKey || apiKey.length < 10) {
    ...
    await supabase.from("email_logs").insert({ ...logEntry, status: "skipped", error: reason }).catch(()=>{});
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      await supabase.from("email_logs").insert({ ...logEntry, status: "failed", error: friendly }).catch(()=>{});
      return { error: friendly };
    }

    const msgId = json?.messageId || json?.id || "sent";
    console.log(`✅ Email → ${to} | Brevo msgId: ${msgId}`);
    await supabase.from("email_logs").insert({ ...logEntry, status: "sent" }).catch(()=>{});
